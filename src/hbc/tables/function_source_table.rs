...
impl FunctionSourceTable {
    ...
    pub fn get_entries(&self) -> Vec<(u32, u32)> {
        self.entries.iter().map(|entry| (entry.function_id, entry.string_id)).collect()
    }
    
    /// Get the source filename for a function ID
    pub fn get_source_filename(&self, function_id: u32, string_table: &StringTable) -> Option<String> {
