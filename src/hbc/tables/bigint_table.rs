...
impl<'a> BigIntTable<'a> {
    ...
    pub fn get(&self, index: u32) -> Result<String, String> {
        ...
        if entry_offset + 8 > self.table.len() as u32 {
            return Err(format!("BigInt table entry {} would exceed table bounds", index));
        }
        ...
        if storage_end > self.storage.len() {
            return Err(format!("BigInt {} storage would exceed bounds: offset={}, length={}, storage_size={}", 
                             index, offset, length, self.storage.len()));
        }
        ...
        let bigints = match self.extract_bigints() {
            ...
            Err(_) => {
                ...
                for i in 0..self.count() {
                    seq.push(self.get_placeholder(i).unwrap_or_else(|| format!("bigint_{}", i)));
                }
                seq
            }
        };
