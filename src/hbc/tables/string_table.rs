...
impl<'a> StringTableEntry<'a> {
    pub fn hex_string(&self) -> String {
        if self.is_utf16 {
            ...
            let hex_string = self.bytes.chunks_exact(2)
            ...
            let hex_string = self.bytes.iter()
    ...
    pub fn string_value(&self) -> String {
        if self.is_utf16 {
            let utf16_bytes = self.bytes.chunks_exact(2).map(|chunk| u16::from_le_bytes([chunk[0], chunk[1]])).collect::<Vec<u16>>();
...
impl<'a> StringTable<'a> {
    ...
    pub fn parse(data: &'a [u8], header: &HbcHeader, offset: &mut usize) -> Result<Self, String> {
        ...
        *offset += string_kinds_size;
        ...
        let identifier_hashes_data = &data[identifier_hashes_start..identifier_hashes_start + identifier_hashes_size];
        *offset += identifier_hashes_size;
        ...
        let small_string_table = &data[small_string_table_start..small_string_table_start + small_string_table_size];
        *offset += small_string_table_size;
        ...
        let overflow_string_table = &data[overflow_string_table_start..overflow_string_table_start + overflow_string_table_size];
        ...
        *offset += storage_size;
        ...
        let small_entries = Self::parse_small_string_table_internal(small_string_table, header.string_count(), header.version())?;
    ...
    pub fn get_entry(&self, string_id: u32) -> Result<StringTableEntry, String> {
        if string_id >= self.string_count {
            return Err(format!("String ID {} out of range (max: {})", string_id, self.string_count - 1));
        }
        ...
        let bytes = if uses_overflow {
            ...
            if offset_bytes < self.overflow_entries.len() as u32 {
                ...
                let byte_length = if is_utf16 { actual_length * 2 } else { actual_length };
            ...
            if end > self.storage.len() {
                return Err(format!(
                    "String storage access out of bounds: offset={}, length={}, storage_size={}",
                    offset_bytes, byte_length, self.storage.len()
                ));
            }
        ...
        let string_kind = self.string_kinds.get(string_id as usize)
            .ok_or(format!("String ID {} out of range (max: {})", string_id, self.string_count - 1))?
            .clone();
    ...
    fn get_uncached(&self, string_id: u32) -> Result<String, String> {
        ...
        if entry.is_utf16 {
            let utf16_bytes = entry.bytes.chunks_exact(2).map(|chunk| u16::from_le_bytes([chunk[0], chunk[1]])).collect::<Vec<u16>>();
    ...
    fn parse_small_string_table_internal(small_string_table: &[u8], string_count: u32, version: u32) -> Result<Vec<SmallStringTableEntry>, String> {
        let mut entries = Vec::new();
        
        for i in 0..string_count {
            let entry_offset = i as usize * 4;
            let entry_bytes: [u8; 4] = small_string_table[entry_offset..entry_offset + 4].try_into().unwrap();
    ...
    fn parse_overflow_string_table_internal(overflow_string_table: &[u8]) -> Result<Vec<OverflowStringTableEntry>, String> {
        let mut entries = Vec::new();
    ...
    fn parse_small_string_table(&self, version: u32) -> Result<Vec<SmallStringTableEntry>, String> {
        Self::parse_small_string_table_internal(self.small_string_table_data, self.string_count, version)
    }
    ...
    pub fn parse_string_kinds(string_kinds_data: &'a [u8]) -> Result<(Vec<StringKind>, Vec<Option<u32>>), String> {
    ...
    pub fn parse_identifier_hashes(identifier_hashes_data: &[u8]) -> Result<Vec<u32>, String> {
        ...
        for i in 0..identifier_hashes_data.len() / 4 {
            let offset = i * 4;
            let hash = u32::from_le_bytes(identifier_hashes_data[offset..offset + 4].try_into().unwrap());
            hashes.push(hash);
        }
...
impl<'a> Serialize for StringTable<'a> {
    ...
    where
        S: serde::Serializer,
    {
        let entries = self.parse_small_string_table(self.version).map_err(serde::ser::Error::custom)?;
