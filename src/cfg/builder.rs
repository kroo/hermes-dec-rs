...
impl CfgBuilder {
    ...
    pub fn build_from_instructions(&mut self, instructions: &[HbcFunctionInstruction], function_index: u32) -> DiGraph<Block, EdgeKind> {
        ...
        self.jump_table.build_for_function(function_index, instructions)
    ...
    fn get_jump_target(&self, instruction: &HbcFunctionInstruction) -> Option<u32> {
        ...
        if let Some(jump_label) = self.jump_table.get_label_by_jump_op_index(instruction.function_index, instruction.instruction_index) {
            // Find the label in the jump table to get the target instruction index
            if let Some(labels) = self.jump_table.get_labels_for_function(instruction.function_index) {
                for label in labels {
    ...
    fn create_blocks(&self, instructions: &[HbcFunctionInstruction], leaders: &HashSet<u32>) -> Vec<Block> {
    ...
    fn add_edges(&mut self, graph: &mut DiGraph<Block, EdgeKind>, _instructions: &[HbcFunctionInstruction]) {
    ...
    fn get_edge_kind(&self, instruction: &HbcFunctionInstruction) -> EdgeKind {
        match &instruction.instruction {
            ...
            UnifiedInstruction::JmpLong { .. } => EdgeKind::Uncond,
            UnifiedInstruction::JmpTrue { .. } |
            UnifiedInstruction::JmpTrueLong { .. } => EdgeKind::True,
            UnifiedInstruction::JmpFalse { .. } |
            UnifiedInstruction::JmpFalseLong { .. } => EdgeKind::False,
    ...
    pub fn analyze_dominators(&self, graph: &DiGraph<Block, EdgeKind>) -> Option<Dominators<NodeIndex>> {
    ...
    pub fn find_natural_loops(&self, graph: &DiGraph<Block, EdgeKind>) -> Vec<(NodeIndex, NodeIndex)> {
    ...
    pub fn find_if_else_joins(&self, graph: &DiGraph<Block, EdgeKind>) -> Vec<NodeIndex> {
        ...
        if let Some(_dominators) = self.analyze_dominators(graph) {
            ...
            for node in graph.node_indices() {
                let predecessors: Vec<NodeIndex> = graph.neighbors_directed(node, petgraph::Direction::Incoming).collect();
    ...
    pub fn find_switch_dispatches(&self, graph: &DiGraph<Block, EdgeKind>) -> Vec<Vec<NodeIndex>> {
        ...
        for node in graph.node_indices() {
            let successors: Vec<NodeIndex> = graph.neighbors_directed(node, petgraph::Direction::Outgoing).collect();
        ...
        if let Some(dominators) = self.analyze_dominators(graph) {
            ...
            for node in graph.node_indices() {
                if let Some(immediate_dominator) = dominators.immediate_dominator(node) {
                    result.push_str(&format!("  Block {} -> Block {}\n", 
                        node.index(), immediate_dominator.index()));
                } else {
