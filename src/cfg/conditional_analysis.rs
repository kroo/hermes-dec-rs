...
fn build_node_mappings_recursive(
    ...
    for (idx, chain) in chains.iter().enumerate() {
        ...
        for branch in &chain.branches {
            // Add condition block
            node_to_chains.entry(branch.condition_block)
                .or_insert_with(Vec::new)
                .push(global_idx);
            
            // Add branch blocks
            for &block in &branch.branch_blocks {
                node_to_chains.entry(block)
                    .or_insert_with(Vec::new)
                    .push(global_idx);
            }
        }
...
impl<'a> ConditionalDetector<'a> {
    ...
    fn find_exit_block(&self) -> Option<NodeIndex> {
        self.graph.node_indices()
            .find(|&n| {
                self.graph[n].is_exit() || 
                self.graph.edges(n).count() == 0
            })
    }
        ...
        for &cond_node in &conditionals {
            ...
            if let Some(mut chain) = self.build_chain_from_hierarchical(cond_node, depth) {
                ...
                for branch in &mut chain.branches {
                    ...
                    let nested = self.detect_chains_in_region(
                        branch_entry, 
                        branch_exit,
                        depth + 1
                    );
                    ...
                    if !nested.is_empty() {
                        ...
                        fn collect_all_nested_blocks(chains: &[ConditionalChain], blocks: &mut HashSet<NodeIndex>) {
                            for chain in chains {
                                for branch in &chain.branches {
                        ...
                        branch.branch_blocks.retain(|&block| !nested_blocks.contains(&block));
    ...
    fn build_chain_from_hierarchical(&mut self, start_node: NodeIndex, depth: usize) -> Option<ConditionalChain> {
        ...
        loop {
            ...
            let local_join = self.find_proper_join_point(current_node, true_target, false_target, depth)?;
    ...
    fn find_immediate_convergence(
        ...
        let true_succs: HashSet<_> = self.graph.edges(true_branch)
            .map(|e| e.target())
            .collect();
        let false_succs: HashSet<_> = self.graph.edges(false_branch)
            .map(|e| e.target())
            .collect();
