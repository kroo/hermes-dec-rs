...
pub fn disasm(input_path: &std::path::Path) -> DecompilerResult<()> {
    ...
    let base_name = input_path
        ...
        .to_string_lossy()
        .to_string();
    let output_filename = input_path.parent().unwrap().join(format!("{}.hasm", base_name));
...
fn generate_hasm_output(hbc_file: &HbcFile, output_path: &std::path::Path) -> DecompilerResult<()> {
    ...
    file.write_all(source_output.as_bytes())
        .map_err(|e| DecompilerError::Io(format!("Failed to write function source table: {}", e)))?;
...
fn generate_header_info(hbc_file: &HbcFile, output: &mut String) -> DecompilerResult<()> {
    ...
    output.push_str(&format!("  Bytecode version number: {}\n", header.version()));
    
    // Format source hash as hex string
    let source_hash_hex = header.source_hash().iter()
        .map(|b| format!("{:02x}", b))
        .collect::<String>();
    ...
    output.push_str(&format!("  String Kind Entry count: {}\n", header.string_kind_count()));
    ...
    output.push_str(&format!("  CommonJS module count: {}\n", header.cjs_module_count()));
    output.push_str(&format!("  CommonJS module count (static): {}\n", 
        if header.cjs_modules_statically_resolved() { header.cjs_module_count() } else { 0 }));
    
    if let Some(function_source_count) = header.function_source_count() {
        output.push_str(&format!("  Function source count: {}\n", function_source_count));
    } else {
        output.push_str("  Function source count: 0\n");
    }
    
    output.push_str("  Bytecode options:\n");
    output.push_str(&format!("    staticBuiltins: {}\n", if header.static_builtins() { 1 } else { 0 }));
    output.push_str(&format!("    cjsModulesStaticallyResolved: {}\n", 
        if header.cjs_modules_statically_resolved() { 1 } else { 0 }));
    output.push_str("\n");
...
fn generate_string_table(hbc_file: &HbcFile, output: &mut String) -> DecompilerResult<()> {
    ...
    for i in 0..hbc_file.strings.count() {
        let entry = hbc_file.strings.get_entry(i as u32).map_err(|e| DecompilerError::Internal {
        ...
        let prefix = if entry.is_identifier.unwrap_or(false) { "i" } else { "s" };
...
fn generate_function_source_table(hbc_file: &HbcFile, output: &mut String) -> DecompilerResult<()> {
    if hbc_file.function_sources.count() > 0 {
        ...
        let strings = hbc_file.strings.extract_strings()
        ...
        for entry in &hbc_file.function_sources.entries {
            if let Some(_source_string) = strings.get(entry.string_id as usize) {
                output.push_str(&format!("  Function ID {} -> s{}\n", 
...
fn generate_function_header(parsed_header: &crate::hbc::tables::function_table::ParsedFunctionHeader, function_name: &str) -> String {
    let header = &parsed_header.header;
    let prohibit_invoke = header.prohibit_invoke();
    let prohibit_invoke_str = if prohibit_invoke == 1 { "NCFunction" } else { "Function" };
    let function_header = format!("{}<{}>{}(", prohibit_invoke_str, function_name, parsed_header.index);
    ...
    let function_header = format!("{}{} symbols):\n", function_header, header.environment_size());
    function_header
}

/// Format a single instruction
fn format_instruction(instruction: &crate::hbc::tables::function_table::HbcFunctionInstruction, hbc_file: &HbcFile) -> String {
    instruction.format_instruction(hbc_file)
}

/// Generate exception handlers string
fn generate_exception_handlers(parsed_header: &crate::hbc::tables::function_table::ParsedFunctionHeader, _jump_table: &crate::hbc::JumpTable) -> String {
    ...
    if !parsed_header.exc_handlers.is_empty() {
        ...
        for (i, handler) in parsed_header.exc_handlers.iter().enumerate() {
            ...
            exception_output.push_str(&format!("{}: start = L{}, end = L{}, target = L{}\n", 
                i, start, end, target));
        }
    }
    exception_output
...
fn generate_single_function(hbc_file: &HbcFile, _idx: usize, parsed_header: &crate::hbc::tables::function_table::ParsedFunctionHeader) -> DecompilerResult<String> {
    ...
    let formatted_instructions: Vec<String> = if instructions.len() >= PARALLEL_THRESHOLD {
        ...
        instructions.par_iter().map(|instruction| {
            let mut result = String::new();
            
            // Check if this instruction should have a label
            if let Some(label_name) = hbc_file.jump_table.get_label_by_inst_index(instruction.function_index, instruction.instruction_index) {
...
fn generate_all_functions(hbc_file: &HbcFile, file: &mut BufWriter<std::fs::File>) -> DecompilerResult<()> {
    ...
    let function_outputs: Vec<DecompilerResult<String>> = hbc_file.functions.parsed_headers
        .par_iter()
        .enumerate()
    ...
    eprintln!("Function generation completed in {:.2}ms ({:.1} functions/second)", 
...
fn generate_bigint_table_section(hbc_file: &HbcFile, output: &mut String) -> DecompilerResult<()> {
    let bigints = hbc_file.bigints.extract_bigints().map_err(|e| DecompilerError::Internal {
    ...
    for (i, value) in bigints.iter().enumerate() {
        ...
        if let Some(entry) = hbc_file.bigints.table.get(i * 8..i * 8 + 8) {
            ...
            let end = if length > 0 { offset + length - 1 } else { offset };
            // Truncate value if too long
            let (display, note) = if value.len() > 30 {
                (&value[..16], format!("... ({} decimal digits)", value.len()))
            } else {
