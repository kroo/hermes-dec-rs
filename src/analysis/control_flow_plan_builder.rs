...
impl<'a> ControlFlowPlanBuilder<'a> {
    ...
    fn build_from_switch_analysis(&mut self, analysis: &crate::cfg::analysis::SwitchAnalysis) -> StructureId {
    ...
    fn build_from_conditional_analysis(&mut self, analysis: &crate::cfg::analysis::ConditionalAnalysis) -> StructureId {
    ...
    fn build_from_loop_analysis(&mut self, analysis: &crate::cfg::analysis::LoopAnalysis) -> StructureId {
    ...
    fn build_case_groups_from_switch_info(&mut self, info: &SwitchInfo, region: &SwitchRegion) -> Vec<crate::analysis::control_flow_plan::CaseGroupStructure> {
        ...
        let mut groups_by_target: BTreeMap<NodeIndex, Vec<&crate::cfg::switch_analysis::CaseInfo>> = BTreeMap::new();
        for case in &info.cases {
            groups_by_target.entry(case.target_block)
                .or_default()
                .push(case);
        }
        ...
        for (target_block, cases_in_group) in groups_by_target {
            ...
            let body = if shared_tail == Some(target_block) {
                ...
                let case_analysis = cases_in_group.iter()
                    .find_map(|case_info| {
            ...
            let switch_cases: Vec<_> = cases_in_group.iter().map(|case_info| {
                crate::analysis::control_flow_plan::SwitchCase {
    ...
    fn build_case_groups_from_region(&mut self, region: &SwitchRegion) -> Vec<crate::analysis::control_flow_plan::CaseGroupStructure> {
        let mut case_groups = Vec::new();
    ...
    fn build_from_case_body_analysis(&mut self, analysis: &crate::cfg::analysis::CaseBodyAnalysis) -> StructureId {
        let mut elements = Vec::new();
        ...
        for &block in &sorted_blocks {
            ...
            if let Some(nested_switch) = analysis.nested_switches.iter()
                .find(|s| s.dispatch == block) 
            {
                ...
                for case in &nested_switch.cases {
                    ...
                    if let Some(ref case_analysis) = nested_switch.case_analyses.get(&case.case_index) {
            ...
            if let Some(chain) = analysis.conditionals.chains.iter()
            ...
            if let Some(loop_info) = analysis.loops.loops.iter()
                .find(|l| l.primary_header() == block)
            {
                ...
                let loop_type = match loop_info.loop_type {
                    crate::cfg::analysis::LoopType::While => crate::analysis::control_flow_plan::LoopType::While,
                    crate::cfg::analysis::LoopType::DoWhile => crate::analysis::control_flow_plan::LoopType::DoWhile,
                    crate::cfg::analysis::LoopType::For => crate::analysis::control_flow_plan::LoopType::For,
                };
    ...
    fn build_sequential_from_blocks(&mut self, blocks: &HashSet<NodeIndex>) -> StructureId {
        ...
        let elements: Vec<_> = sorted_blocks.iter()
    ...
    fn find_next_sequential_block(&self, block: NodeIndex) -> Option<NodeIndex> {
        // Get successors
        let successors: Vec<_> = self.cfg.graph()
            .edges(block)
    ...
    fn detect_fallthrough_patterns(
        ...
        let switch_case_groups: Vec<crate::cfg::switch_analysis::CaseGroup> = case_groups.iter()
            .filter_map(|group| {
        ...
        for (i, fallthrough_target) in fallthrough_analysis.fallthrough_targets.iter().enumerate() {
            if let Some(target_idx) = fallthrough_target {
                if i < case_groups.len() {
                    let duplication_context = crate::cfg::ssa::types::DuplicationContext::SwitchFallthrough {
                        from_case_index: i,
                    ...
                    let blocks_to_duplicate = if *target_idx < case_groups.len() {
                        if let Some(target_structure) = self.plan.get_structure(case_groups[*target_idx].body) {
                    ...
                    case_groups[i].fallthrough = Some(crate::analysis::control_flow_plan::FallthroughInfo {
                        to_case_index: *target_idx,
                        blocks_to_duplicate,
                        duplication_context,
                    });
