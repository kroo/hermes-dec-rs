...
impl fmt::Display for GlobalSSAError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            GlobalSSAError::ParentNotFound { func_idx, parent_level } => {
                write!(f, "Parent function not found for function {} at level {}", func_idx, parent_level)
            }
            GlobalSSAError::MissingSSAAnalysis { func_idx, context } => {
                write!(f, "SSA analysis missing for function {}: {}", func_idx, context)
            }
...
impl GlobalSSAAnalyzer {
    ...
    fn ensure_function_analyzed_lazy_impl(&self, func_idx: u32, visited: &mut std::collections::HashSet<u32>) {
        // Prevent infinite recursion
        if !visited.insert(func_idx) {
            log::warn!("GlobalSSA: Circular dependency detected for function {}", func_idx);
            return;
        }
        ...
        let mut ssa = match construct_ssa(&cfg, func_idx) {
            Ok(ssa) => ssa,
            Err(e) => {
                panic!("GlobalSSA: Failed to construct SSA for function {}: {:?}", func_idx, e);
            }
        };
        
        // Extract function environment register
        if let Some(env_reg) = ssa.environment_info.function_env_register {
            self.function_environments.lock().unwrap().insert(func_idx, env_reg);
        }
    ...
    fn scan_closure_relationships(&mut self, hbc_file: &HbcFile) -> Result<(), GlobalSSAError> {
        ...
        for parent_idx in 0..hbc_file.functions.count() {
            match hbc_file.functions.get(parent_idx, hbc_file) {
                ...
                Err(e) => {
                    log::warn!("GlobalSSA: Failed to get function {} for closure scanning: {:?}", parent_idx, e);
    ...
    pub fn get_function_analysis(&self, func_idx: u32) -> Option<SSAAnalysis> {
        self.function_analyses.lock().unwrap().get(&func_idx).cloned()
    }
    ...
    pub fn resolve_variable_name(&self, function: u32, env_reg: u8, slot: u8) -> String {
        log::trace!("GlobalSSA: Resolving variable name for function {}, env_reg {}, slot {}", function, env_reg, slot);
        ...
        if let Some(source_func) = source_func {
            ...
            if source_func == function {
                ...
                {
                    log::trace!("GlobalSSA: Resolved from parent function {}: {}", source_func, name);
                    return name.clone();
                } else {
            ...
            log::trace!("GlobalSSA: Could not resolve parent function at level {} (expected for top-level)", resolution_level);
        }
    ...
    pub fn get_variable_name_for_load(
        &self,
        function: u32,
        env_reg: u8,
        slot: u8,
        ...
        let mut slots_to_resolve = Vec::new();
        {
            ...
            if let Some(ssa) = function_analyses.get(&func_idx) {
                if let Some(env_reg) = ssa.environment_info.function_env_register {
                    ...
                    log::debug!("GlobalSSA: Function {} has no environment register", func_idx);
                }
            } else {
