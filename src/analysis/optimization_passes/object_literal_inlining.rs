...
use crate::analysis::value_tracking::{
    EscapeAnalyzer, LiteralReconstructor, PatternDetector, ReconstructionResult, TrackedValue,
};
...
pub fn perform_object_literal_inlining(ctx: &mut OptimizationContext) {
    ...
    let escape_analyzer =
        EscapeAnalyzer::new(&ctx.function_analysis.ssa, &ctx.function_analysis.cfg);
    ...
    for (def_site, ssa_value) in &ctx.function_analysis.ssa.ssa_values {
        ...
        } = &tracked_value
        {
            ...
            let pattern =
                PatternDetector::detect_pattern(base_type, mutations, *creation_pc as u32);
            if !PatternDetector::is_safe_to_inline(&pattern) {
                debug!(
                    "Object {} at PC {} has unsafe pattern",
                    ssa_value, creation_pc
                );
                continue;
            }
            ...
            let reconstructor =
                LiteralReconstructor::new(&ctx.function_analysis.ssa, &ctx.function_analysis.cfg);
            let reconstruction = reconstructor.reconstruct(&tracked_value, ssa_value);

            match reconstruction {
                ReconstructionResult::Literal(js_code) => {
                    debug!(
                        "Object {} at PC {} can be inlined as literal: {}",
                        ssa_value, creation_pc, js_code
                    );

                    // Determine the safe inlining position
                    let emit_position =
                        determine_emit_position(ctx, ssa_value, mutations, *creation_pc);

                    objects_to_inline.push((
                        ssa_value.clone(),
                        tracked_value.clone(),
                        emit_position,
                    ));
                ...
                ReconstructionResult::Partial { .. } => {
                    debug!(
                        "Object {} at PC {} has partial reconstruction",
                        ssa_value, creation_pc
                    );
                ...
                ReconstructionResult::CannotReconstruct(reason) => {
                    debug!(
                        "Object {} at PC {} cannot be reconstructed: {}",
                        ssa_value, creation_pc, reason
                    );
                }
            }
        }
...
fn is_value_safe_at_creation(
    ...
    match value {
        ...
        TrackedValue::Phi { ssa_value } => ssa_value.def_site.instruction_idx.value() < creation_pc,
...
fn apply_object_literal_inlining(
        ...
        for mutation in mutations {
            ...
            if let Some(block_id) = find_block_for_pc(ctx, mutation.pc) {
                ...
                ctx.plan
                    ...
                    .insert((block_id, InstructionIndex(mutation.pc)));
    ...
    for use_site in uses {
        ...
        if let TrackedValue::MutableObject { mutations, .. } = &tracked_value {
            if mutations
                .iter()
                .any(|m| m.pc == use_site.instruction_idx.value())
            {
                continue;
            }
        }
        ...
        if !ctx.plan.use_strategies.contains_key(&key) {
            ctx.plan
                .use_strategies
                .insert(key, UseStrategy::UseVariable);
        }
    }
}
