...
use crate::analysis::value_tracking::types::{
    MutationKind, ObjectBaseType, ObjectMutation, TrackedValue,
};
...
pub enum ConstructionPattern {
    ...
    SimpleArrayLiteral { elements: Vec<ArrayElement> },
...
impl PatternDetector {
    ...
    fn are_mutations_immediate(mutations: &[ObjectMutation], creation_pc: u32) -> bool {
        ...
        let min_pc = mutations
            .iter()
            .map(|m| m.pc)
            .min()
            .unwrap_or(creation_pc_usize);
        let max_pc = mutations
            .iter()
            .map(|m| m.pc)
            .max()
            .unwrap_or(creation_pc_usize);
            ...
            match &mutation.kind {
                ...
                | MutationKind::PropertyDefine { key, value } => {
                    ...
                    if let TrackedValue::Constant(
                        crate::analysis::value_tracking::types::ConstantValue::String(key_str),
                    ) = &**key
                    {
                        ...
                        let prop_value = match &**value {
                            TrackedValue::Constant(c) => {
                                PropertyValue::Constant(format!("{:?}", c))
                            }
                            TrackedValue::Parameter { index, .. } => {
                                PropertyValue::Register(*index as u8)
                            }
    ...
    fn detect_array_pattern(mutations: &[ObjectMutation]) -> ConstructionPattern {
        ...
        for mutation in mutations {
            match &mutation.kind {
                MutationKind::ArraySet { index, value } => {
                    ...
                    if let TrackedValue::Constant(
                        crate::analysis::value_tracking::types::ConstantValue::Number(n),
                    ) = &**index
                    {
                        let idx = *n as usize;
                        ...
                        let elem = match &**value {
                            ...
                            TrackedValue::Parameter { index, .. } => {
                                ArrayElement::Register(*index as u8)
                            }
        ...
        if is_sparse {
            ...
            let has_internal_holes = elements
                .iter()
                .take(last_non_hole)
...
mod tests {
    ...
    fn test_simple_object_pattern() {
        ...
        let pattern = PatternDetector::detect_pattern(&ObjectBaseType::Object, &mutations, 1);
