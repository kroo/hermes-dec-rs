...
impl<'a> EscapeAnalyzer<'a> {
    ...
    fn is_safe_to_inline(
        &self,
        escape_result: &EscapeAnalysisResult,
        object_def: &RegisterDef,
    ) -> bool {
        // If it only escapes via return, it's always safe to inline
        if escape_result.reasons.len() == 1
            && matches!(escape_result.reasons[0], EscapeReason::Returned)
        {
            return true;
        }
    ...
    fn check_use_for_escape(&self, use_site: &RegisterUse, result: &mut EscapeAnalysisResult) {
            ...
            .find(|i| i.instruction_index == use_site.instruction_idx)
        {
            ...
            let operand_position =
                Self::find_operand_position(&instr_info.instruction, use_site.register);
    ...
    pub fn instruction_causes_escape(
        ...
        match instruction {
            // Object is returned from the function
            Ret { .. } if operand_position == 0 => Some(EscapeReason::Returned),

            // Object is thrown as an exception
            Throw { .. } if operand_position == 0 => Some(EscapeReason::Thrown),
            ...
            PutByVal { .. } | PutOwnByVal { .. } if operand_position == 2 => {
                Some(EscapeReason::StoredInContainer)
            }

            // Object is stored by index
            PutOwnByIndex { .. } | PutOwnByIndexL { .. } if operand_position == 1 => {
                Some(EscapeReason::StoredInContainer)
            }

            // Object is stored to environment (closure capture)
            StoreToEnvironment { .. } | StoreToEnvironmentL { .. } if operand_position == 0 => {
                Some(EscapeReason::CapturedByClosure)
            }

            // Object is stored as an N-property
            StoreNPToEnvironment { .. } | StoreNPToEnvironmentL { .. } if operand_position == 0 => {
                Some(EscapeReason::CapturedByClosure)
            }
