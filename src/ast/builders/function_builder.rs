...
use oxc_ast::{
    ast::{BindingPatternKind, Declaration, Program, Statement},
    ...
    pub fn build_function_with_comments(
        ...
        for (_i, param_name) in params.iter().enumerate() {
            ...
            let binding_name = self
                .ast_builder
                .binding_identifier(param_span, self.ast_builder.atom(param_name));
        ...
        let id = self
            .ast_builder
            .binding_identifier(Span::default(), self.ast_builder.atom(name));
        ...
        let (comments, synthetic_source) = self
            .comment_manager
            .finalize_comments(position_assigner.get_address_positions());
...
pub fn build_function_program<'a>(
    ...
    builder.build_function_with_comments(name, params, default_params, statements, function_type)
}

/// Generate final JavaScript code with comments using the address-based system
pub fn generate_code_with_comments<'a>(program: &Program<'a>, synthetic_source: &str) -> String {
