...
fn test_version_registry_manual() {
    ...
    registry.add_version(HbcVersion::new(
        80,
        Some("v0.8.0".to_string()),
        None,
        Some("2021-04-29".to_string()),
    ));
    registry.add_version(HbcVersion::new(
        95,
        None,
        Some("f6b56d3".to_string()),
        Some("2023-03-29".to_string()),
    ));
    registry.add_version(HbcVersion::new(
        96,
        None,
        Some("e0fc671".to_string()),
        Some("2023-08-16".to_string()),
    ));
...
fn test_version_registry_from_json() {
    ...
    let versions_content =
        std::fs::read_to_string(versions_file).expect("Failed to read versions.json");
    ...
    let versions_file: VersionsFile =
        serde_json::from_str(&versions_content).expect("Failed to parse versions.json");
