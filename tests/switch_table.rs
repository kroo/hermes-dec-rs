...
fn test_switch_table_parsing() {
    ...
    let switch_tables = hbc_file.switch_tables.get_switch_tables_for_function(1).unwrap();
...
fn test_switch_table_by_instruction() {
    ...
    let switch_table = hbc_file.switch_tables.get_switch_table_by_instruction(1, 1).unwrap();
    ...
    assert!(hbc_file.switch_tables.get_switch_table_by_instruction(1, 99).is_none());
}
...
fn test_no_switch_tables() {
    ...
    assert!(hbc_file.switch_tables.get_switch_tables_for_function(0).is_none());
}
...
fn test_dense_switch_expected_structure() {
    ...
    for i in 0..=20 {
        assert!(switch_table.get_case_by_value(i).is_some(), "Case {} not found", i);
        assert!(switch_table.is_value_in_range(i), "Value {} not in range", i);
        assert!(switch_table.get_case_for_value(i).is_some(), "Case {} not found via get_case_for_value", i);
    }
    ...
    let instructions = hbc_file.functions.get_instructions(1).expect("Failed to get instructions for function 1");
    ...
    for (case_value, expected_string) in expected_case_strings.iter() {
        let case = switch_table.get_case_by_value(*case_value).expect(&format!("Case {} not found", case_value));
        
        // Get the target instruction index
        let target_instruction_index = case.target_instruction_index.expect(&format!(
            "Case {} has no target instruction index", case_value
        ));
        ...
        assert!(
            ...
            case_value, target_instruction_index, instructions.len()
        );
    ...
    let default_instruction_index = switch_table.default_instruction_index.expect("Default case has no instruction index");
    assert!(
        default_instruction_index < instructions.len() as u32,
        "Default case target instruction index {} out of bounds ({} instructions)",
        default_instruction_index, instructions.len()
    );
    
    let default_instruction = &instructions[default_instruction_index as usize];
    match &default_instruction.instruction {
        hermes_dec_rs::generated::unified_instructions::UnifiedInstruction::LoadConstString { operand_0: _register, operand_1: string_id } => {
            let actual_string = hbc_file.strings.get(*string_id as u32).expect("Failed to get string for default case");
            assert_eq!(actual_string, "other", "Default case points to string '{}', expected 'other'", actual_string);
        },
        other_instruction => {
            panic!("Default case target instruction is {:?}, expected LoadConstString", other_instruction);
        }
    }

    // Verify a few specific cases with detailed logging
    let test_cases = [(0, "zero"), (5, "five"), (10, "ten"), (15, "fifteen"), (20, "twenty")];
...
fn demonstrate_switch_table_parsing() {
    ...
    for function_index in 0..hbc_file.header.function_count() {
        ...
        if hbc_file.switch_tables.has_switch_tables(function_index) {
            let switch_table_count = hbc_file.switch_tables.get_switch_table_count(function_index);
            println!("  ✓ Switch tables: {} found", switch_table_count);
            
            if let Some(switch_tables) = hbc_file.switch_tables.get_switch_tables_for_function(function_index) {
                for (i, switch_table) in switch_tables.iter().enumerate() {
                    println!("  Switch table {}:", i);
                    println!("    Range: {} to {} (inclusive)", switch_table.min_value, switch_table.max_value);
                    println!("    Cases: {} (expected: {})", switch_table.case_count(), switch_table.expected_case_count());
                    ...
                    println!("    Switch instruction index: {}", switch_table.switch_instruction_index);
                    
                    // Show some case examples
                    if !switch_table.cases.is_empty() {
                        println!("    Sample cases:");
                        for case in switch_table.cases.iter().take(5) {
                            println!("      Case {}: target_offset={}", case.value, case.target_offset);
                        }
    ...
    if let Some(switch_table) = hbc_file.switch_tables.get_switch_table_by_instruction(1, 1) {
        ...
        println!("  Range: {} to {}", switch_table.min_value, switch_table.max_value);
    ...
    // Test non-existent switch table
    if hbc_file.switch_tables.get_switch_table_by_instruction(1, 99).is_none() {
