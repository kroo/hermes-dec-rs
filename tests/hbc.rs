use hermes_dec_rs::hbc::{HbcFile, HBC_MAGIC};
use std::fs;
...
fn test_hermes_dec_sample_hbc() {
    let data =
        fs::read("data/hermes_dec_sample.hbc").expect("Failed to read hermes_dec_sample.hbc");
    ...
    let _json =
        serde_json::to_string(&hbc_file).expect("Failed to serialize hermes_dec_sample.hbc");
}
...
fn test_bigints_v96_hbc() {
    ...
    let bigints = hbc_file
        .bigints
        .extract_bigints()
        .expect("Failed to extract bigints");
...
fn test_array_constants_v96_hbc() {
    let data =
        fs::read("data/array_constants_v96.hbc").expect("Failed to read array_constants_v96.hbc");
    ...
    let _json =
        serde_json::to_string(&hbc_file).expect("Failed to serialize array_constants_v96.hbc");
}

#[test]
fn test_array_constants_v90_hbc() {
    let data =
        fs::read("data/array_constants_v90.hbc").expect("Failed to read array_constants_v90.hbc");
    ...
    let _json =
        serde_json::to_string(&hbc_file).expect("Failed to serialize array_constants_v90.hbc");
}
...
fn test_cjs_v96_hbc() {
    ...
    for (i, module) in hbc_file.cjs_modules.entries.iter().enumerate() {
        assert!(
            module.symbol_id > 0,
            "CJS module {} should have a symbol ID",
            i
        );
...
fn test_cjs_show_source_hbc() {
    ...
    for source in &hbc_file.function_sources.entries {
        assert!(
            source.function_id > 0,
            "Function source should have valid function ID"
        );
        assert!(
            source.string_id > 0,
            "Function source should have valid string ID"
        );
    }
    ...
    assert!(
        has_overflow_strings,
        "Should have overflow strings with long content"
    );
...
fn test_string_table_overflow_handling() {
    ...
    let strings = hbc_file
        ...
        .extract_strings()
        .expect("Failed to extract strings");
    let long_strings: Vec<&String> = strings.iter().filter(|s| s.len() > 100).collect();
    ...
    for string in long_strings {
        assert!(
            !string.contains('\u{0}'),
            "Overflow string should not contain null bytes"
        );
        assert!(
            string.chars().any(|c| c.is_ascii_alphabetic()),
            "Overflow string should contain readable text: {}",
            string
        );
    }
}
...
fn test_utf16_string_decoding() {
    ...
    for (i, string) in strings.iter().enumerate() {
        ...
        assert!(
            !string.contains('\u{0}'),
            "String {} should not contain null bytes",
            i
        );

        // Should be valid UTF-8
        assert!(
            string.is_char_boundary(0),
        ...
        if string.len() > 50 {
            assert!(
                string.chars().any(|c| c.is_ascii_alphabetic()),
                "Long string {} should contain readable text: {}",
                i,
                string
            );
        }
...
fn test_bigint_decimal_representation() {
    ...
    for (i, bigint) in bigints.iter().enumerate() {
        ...
        assert!(
            !bigint.starts_with("bigint_"),
            "BigInt {} should not be placeholder: {}",
            i,
            bigint
        );
        ...
        assert!(
            bigint.chars().all(|c| c.is_ascii_digit() || c == '-'),
            "BigInt {} should be decimal: {}",
            i,
            bigint
        );
...
fn test_cjs_module_symbol_resolution() {
    ...
    for (i, module) in hbc_file.cjs_modules.entries.iter().enumerate() {
        ...
        assert!(
            module.symbol_id > 0,
            "CJS module {} should have valid symbol ID",
            i
        );
        assert!(
            module.offset > 0,
            "CJS module {} should have valid offset",
            i
        );
    }
}
